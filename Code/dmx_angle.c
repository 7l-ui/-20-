...
void Init_Gyro_Offset(void)      
{
    ...
    for (i = 0; i < 100;)
    {
        ...
        Get_Gyro_ICM42688();
				...
				// 舍弃离谱数据
				if(icm42688_gyro_x > -10 && icm42688_gyro_x < 10
					&& icm42688_gyro_y > -10 && icm42688_gyro_y < 10
				  && icm42688_gyro_z > -10 && icm42688_gyro_z < 10)
				{
					 ++i;
           gyro_offset.x_data += icm42688_gyro_x;
           gyro_offset.y_data += icm42688_gyro_y;
           gyro_offset.z_data += icm42688_gyro_z;
				}
...
void Get_Angles_ICM(void)
{
    ...
    // 四元数计算欧拉角
    euler_angle.pitch = asin(-2 * q1 * q3 + 2 * q0 * q2) * 180 / M_PI - 180;
...
static void Get_Values_ICM(void)
{
    ...
    // 一阶低通滤波
    icm_data.acc_x = (((float) icm42688_acc_x) * alpha)  + last_acc[0] * (1 - alpha);
    icm_data.acc_y = (((float) icm42688_acc_y) * alpha)  + last_acc[1] * (1 - alpha);
    icm_data.acc_z = (((float) icm42688_acc_z) * alpha)  + last_acc[2] * (1 - alpha);
    ...
    // 陀螺仪角度转弧度
    icm_data.gyro_x = ((float) icm42688_gyro_x - gyro_offset.x_data) * M_PI / 180 / 16.4f;
    icm_data.gyro_y = ((float) icm42688_gyro_y - gyro_offset.y_data) * M_PI / 180 / 16.4f;
    icm_data.gyro_z = ((float) icm42688_gyro_z - gyro_offset.z_data) * M_PI / 180 / 16.4f;
}
